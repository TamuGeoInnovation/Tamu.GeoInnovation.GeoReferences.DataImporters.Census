...
namespace USC.GISResearchLab.Common.Census.Tiger2010.FileLayouts.StateFiles.AbstractClasses
{
    public abstract class AbstractTiger2010StateFileLayout: AbstractTiger2010FileLayout
    {

        public AbstractTiger2010StateFileLayout(string tableName)
            : base(tableName) { }
        ...
        public override DataTable GetDataTableFromZipFile(string zipFileDirectory)
        {
            ...
            try
            {
                ...
                string zipFileName = "tl_2010_" + stateFips + "_" + FileName;
        ...
        public override IDataReader GetDataReaderFromZipFile(string zipFileDirectory)
        {
            ...
            try
            {
                ...
                string zipFileInnerDirectory = Path.Combine(zipFileDirectory, stateFips);
                string zipFileLocation = Path.Combine(zipFileInnerDirectory, zipFileName);
                if (FileUtils.FileExists(zipFileLocation))
                {
                    string now = DateTime.Now.Millisecond.ToString();
                    string fileName = FileUtils.GetFileNameWithoutExtension(zipFileLocation);
